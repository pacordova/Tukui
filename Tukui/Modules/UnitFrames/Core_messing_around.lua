...
function TukuiUnitFrames:GetRaidFramesAttributes(i)
...
function TukuiUnitFrames:CreateUnits()
	...
	if C.UnitFrames.Enable then
		...
		if C.Raid.Enable then
			local Raid = {}
			for i=1,8 do
				Raid[i] = oUF:SpawnHeader(TukuiUnitFrames:GetRaidFramesAttributes(i))
				Raid[i]:SetParent(UIParent)
			end
			for i=1,8 do
				if i==1 then
					Raid[i]:Point("TOPLEFT", UIParent, "TOPLEFT", 12, -180)
				elseif i==5 then
					Raid[i]:Point("TOPLEFT", UIParent, "TOPLEFT", 12, -600)
				else
					Raid[i]:Point("TOPLEFT", Raid[i-1], "TOPRIGHT", 12, 0)
				end				
			end
			Raid=Raid[1]
